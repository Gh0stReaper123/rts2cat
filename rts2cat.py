...
radioSelection = guizero.Combo(
    root, options=radioOptions, grid=[1, 0], align="left")
...
def comCheck(baudArg, radioPortArg, loopbackPortArg, stopBitsArg, parityArg, byteSizeArg, TX, RX):
    ...
    try:
        ...
        cat = serial.Serial(port=radioPortArg, baudrate=baudArg,
                            stopbits=stopBitsArg, parity=parityArg, bytesize=byteSizeArg)
...
radioBaudSelection = guizero.Combo(
    root, options=radioCommands[radioSelection.value]["Baud"], grid=[1, 1], align="left")
radioBaudLabel = guizero.Text(
    root, text="Radio Baud: ", grid=[0, 1], align="left")
# Initialises the radioBaudSelection drop-down menu and the corrisponding label

catComSelection = guizero.Combo(
    root, options=comPorts, grid=[1, 2], align="left")
guizero.Text(root, text="Radio Port: ", grid=[0, 2], align="left")
tncComSelection = guizero.Combo(
    root, options=comPorts, grid=[1, 3], align="left")
...
def startCom():
    ...
    TxRxLoopThread = threading.Thread(target=comCheck,
                                      args=(int(radioBaudSelection.value),  # baudArg
                                            catComSelection.value,  # radioPortArg
                                            tncComSelection.value,  # loopbackPortArg
                                            # stopBitsArg
                                            radioCommands[radioSelection.value]["stopBits"],
                                            # parityArg
                                            radioCommands[radioSelection.value]["parity"],
                                            # byteSizeArg
                                            radioCommands[radioSelection.value]["byteSize"],
                                            # TX
                                            radioCommands[radioSelection.value]["TX"],
                                            radioCommands[radioSelection.value]["RX"]),  # RX
                                      daemon=True)
...
StartTxButton = guizero.PushButton(
    root, text="Enable TX", command=startCom, grid=[0, 4])
EndTxButton = guizero.PushButton(
    root, text="Disable TX", command=stopCom, grid=[1, 4])
EndTxButton.disable()
